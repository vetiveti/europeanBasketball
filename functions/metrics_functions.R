...
wp <- function(player,team,marg_values,DREBMATE,tASTpm,pos_avg){
    ...
    # foul percentage
    team_for_merge <- team %>% 
        dplyr::select(team, opp_ftm, pf, game, year) %>% 
        rename(pf_t = pf)
    
    player_for_merge <- player %>% 
        dplyr::select(player, team, pf,year,game) %>% 
        mutate(pf_p = pf)
    ...
    player_perT <- player_perT %>% 
        group_by(team,year,game) %>% 
        mutate(PF_perc = pf_p1 / pf_t,
               pf_p_new = opp_ftm * PF_perc) %>% 
        ungroup() %>% 
        dplyr::select(player,team,year,game,pf_p_new)
    
    player_totals <- merge(player, player_perT,
                           by = c("player","team","year","game")) %>% 
    ...
    wins_produced <- player_totals %>% 
        select(player,team,year,game,min_p,Pos.,total_production) %>%
    ...
    DREB_adjustment <- merge(player_totals, team,
                             ...
                             all = TRUE) %>%
        arrange(player) %>%
        dplyr::select(player,team,year,game,min_p,min,drb.x,drb.y)
    ...
    assist_adjustment <- merge(player_totals, team,
        ...
        arrange(player) %>%
        dplyr::select(1:2,year,game,min,min_p,ast.x,ast.y, fga.x)
    ...
    team_adjustment <- merge(team_adjustment, team,
                             ...
                             all = TRUE) %>% 
        arrange(player) %>%
        dplyr::select(1:3,game,min,min_p,opp_p3m,opp_p2m,opp_tov,REBTM,TTOV,blk.y,stl.y,tov.y) %>% 
    ...
    wins_produced <- merge(wins_produced, pos_avg,
        ...
        dplyr::select(player,team,year,game,min_p,wp_total,wp_pm)
...
ws <- function(player,team){
    ...
    a <- merge(player,team, by = c("team","year","game"), suffix = c("_p","_t")) %>% 
        rename(min_t = min) %>% 
        filter(min_p > 0.0)
    ...
    scoring_poss <- a %>% 
        mutate(qAST = ((min_p / (min_t / 5)) * (1.14 * ((ast_t - ast_p) / fgm_t))) + 
                   (ifelse(min_p == 1.6,0.001,(((ast_t / min_t) * min_p * 5 - ast_p) / ((fgm_t / min_t) * min_p * 5 - fgm_p)) * (1 - (min_p / (min_t / 5))))),
    ...
    lg_avg <- poss_t %>% 
        ...
        dplyr::select(year,game,ppp,ppg,pace)
    
    #******************************************************************************#
    # WIN SHARES:----
    # offensive win shares:----
    oRTG_merg <- merge(oRTG,lg_avg, by = c("year","game"))
    ...
    dRTG_merg <- merge(dRTG,lg_avg, by = c("year","game"))
    ...
    #******************************************************************************#
    # total win shares----
    win_shares_merg <- merge(oWin_shares,dWin_shares, by = c("year","team","player","min_p","game"))
    Win_Shares <- win_shares_merg %>% 
        mutate(win_shares = owin_share + dwin_share) %>% 
        dplyr::select(player, team,year,game, min_p, owin_share, dwin_share, win_shares) %>% 
        arrange(-win_shares)
}

BPM <- function(player,team,pos_role){
    # Merge data 
    player_stats <-merge(player,team, by = c("team","year","game"), suffix = c("_p","_t")) %>% 
        filter(min_p > 0.0)
    ...
    player <- merge(player,pos_role,
                    by = c("player","team","year"), all = TRUE)
    
    BPM <- dplyr::select(player,
                  player,team,year,game,min_p,pts_100:min_pct,pos_final,
                  role_final,ORtg_t,DRtg_t,pace_t,G_tot)
